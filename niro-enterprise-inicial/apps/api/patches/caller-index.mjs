...
export class VoipClient {
    ...
    connect = async () => {
        ...
        await new Promise((resolveOpen, rejectOpen) => {
            ...
            const maxRetries = 5;
            const connectSocket = () => {
                if (this.#disposed) { rejectOpen(new Error("Call client closed")); return; }
                this.#sock = createSocket();
                this.#sock.ev.on("creds.update", saveCreds);
                this.#sock.ev.on("connection.update", (update) => {
                    if (update.qr) {
                        rejectOpen(new Error("La sesión de llamadas requiere volver a vincular WhatsApp"));
                        this.#sock?.end?.();
                        return;
                    }
                    if (false && update.qr) {
        ...
        if (this.#disposed) throw new Error("Call client closed");
    ...
    #handleAudioCaptureStart = () => {
        ...
        this.#feeder = new AudioFeeder(this.#captureSampleRate, this.#captureChannels, this.#captureFramesPerChunk, (chunk) => {
            ...
            canPlay: () => this.#activeCall?.state === CallState.Active,
            onStarted: () => this.#activeCall?.emit("audio-started"),
            onEnd: () => this.#activeCall?.end("audio_complete"),
            onError: (error) => {
                this.#activeCall?.emit("provider-error", error);
                this.#activeCall?.end("audio_error");
            },
        });
        this.#feeder.start();
    };
