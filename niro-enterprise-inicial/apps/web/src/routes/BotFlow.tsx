...
const PALETTE_TYPES: BotNodeType[] = ['message', 'keyword', 'condition', 'ai', 'crm', 'agent', 'end'];
...
export function BotFlow() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setSelectedNodeId(flowData.flow.nodes[0]?.id || '');
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      actions: nodes.filter((node) => ['crm', 'agent'].includes(node.type)).length,
  ...
  const canvasSize = useMemo(() => {
    ...
    return { width: Math.max(1400, maxX), height: Math.max(800, maxY) };
  ...
  function arrangeNodes() {
    ...
    rows.forEach((nodes, level) => nodes.forEach((node, row) => positions.set(node.id, { x: 65 + level * 270, y: 55 + row * 145 })));
    updateFlow({ nodes: flow.nodes.map((node) => ({ ...node, position: positions.get(node.id) || node.position })) });
    setNotice('Flujo ordenado. Guardá el flujo para conservar la distribución.');
  }
...
function PaletteButton({ type, onAdd }: { type: BotNodeType; onAdd: () => void }) {
  const def = NODE_DEFS[type];
  return <button type="button" draggable onDragStart={(event) => event.dataTransfer.setData('text/plain', `palette:${type}`)} onClick={onAdd} className="bot-palette-button"><span style={{ color: def.color }}>{def.icon}</span>{def.label}<b>＋</b></button>;
}
