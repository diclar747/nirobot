...
function AudioLibrary({ audios, tts, onChange }: { audios: CallAudio[]; tts: CallTtsInfo; onChange: () => void }) {
  ...
  return <section className="call-library-grid"><div className="call-table-card"><div className="call-section-heading"><div><h2>Biblioteca de audios</h2><p>Audios MP3 listos para asociar a una campaña o reproducir durante la llamada.</p></div></div>{audios.length ? <div className="call-audio-list">{audios.map((audio) => <div className="call-audio-row" key={audio.id}><span className={`call-audio-icon ${audio.source === 'AI' ? 'ai' : ''}`}>{audio.source === 'AI' ? '✦' : '♫'}</span><div><strong>{audio.name}</strong><small>{audio.source === 'AI' ? `Generado con IA${audio.voice ? ` · ${audio.voice}` : ''}` : 'Audio subido'} · {formatBytes(audio.size)}</small></div><audio controls preload="none" src={audio.fileUrl} /></div>)}</div> : <EmptyState text="Generá o subí el primer audio para empezar" />}</div><div className="call-upload-card"><div className="call-audio-mode"><button type="button" className={mode === 'ai' ? 'active' : ''} onClick={() => setMode('ai')}>✦ Generar con IA</button><button type="button" className={mode === 'upload' ? 'active' : ''} onClick={() => setMode('upload')}>♫ Subir archivo</button></div>{mode === 'ai' ? <form onSubmit={generate}><span className="call-form-kicker">NUEVO AUDIO INTELIGENTE</span><h2>Crear voz con IA</h2><p>Escribí el guion y guardalo como MP3 para usarlo en llamadas reales.</p><input className="input" placeholder="Nombre del audio" value={aiName} onChange={(e) => setAiName(e.target.value)} /><textarea className="input call-ai-text" maxLength={5000} placeholder="Hola, te llamamos de Niro para contarte..." value={aiText} onChange={(e) => setAiText(e.target.value)} />{tts.voices.length > 0 && <label className="field"><span>Voz</span><select className="input" value={aiVoice} onChange={(e) => setAiVoice(e.target.value)}>{tts.voices.map((voice) => <option key={voice}>{voice}</option>)}</select></label>}<label className="field"><span>Idioma</span><select className="input" value={aiLanguage} onChange={(e) => setAiLanguage(e.target.value)}><option value="es-PY">Español · Paraguay</option><option value="es-419">Español · Latinoamérica</option><option value="es-ES">Español · España</option></select></label>{!tts.configured && <div className="call-inline-note"><strong>Falta activar el proveedor de voz</strong><span>{tts.reason}</span></div>}{error && <div className="call-inline-error">{error}</div>}<button className="btn" disabled={!aiText.trim() || !tts.configured || busy}>{busy ? 'Generando MP3…' : 'Generar y guardar audio'}</button></form> : <form onSubmit={upload}><span className="call-form-kicker">NUEVO RECURSO</span><h2>Agregar audio</h2><p>El proveedor real requiere que el servidor tenga ffmpeg para decodificar y remuestrear la fuente.</p><input className="input" placeholder="Nombre del audio" value={name} onChange={(e) => setName(e.target.value)} /><label className="call-file-picker"><input type="file" accept="audio/mpeg,audio/wav,audio/x-wav" onChange={(e) => setFile(e.target.files?.[0] || null)} />{file ? `♫ ${file.name}` : 'Elegir MP3 o WAV'}</label>{error && <div className="call-inline-error">{error}</div>}<button className="btn" disabled={!file || busy}>{busy ? 'Cargando…' : 'Subir audio'}</button></form>}</div></section>;
}
